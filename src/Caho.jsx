...
import BelowImageCaho from "../src/assets/Img/cahotech-about.webp";
...
const Caho = () => {
  ...
  const searchUser = async (data) => {
    ...
    try {
      const response = await fetch(`${SHEETDB_API}/search?email=${data.name}`);
  ...
  const updateUserStatus = async (status) => {
    ...
    try {
      const response = await fetch(
        `${SHEETDB_API}/email/${selectedUser.email}`,
        {
          method: "PATCH",
      ...
      if (response.ok) {
        ...
        setUserData((prevData) =>
          prevData.map((user) =>
            user.id === selectedUser.email ? updatedUser : user
          )
        );
      } else {
        Swal.fire({
    ...
    <div>
      ...
      <div className="odm">
        <div className="container">
          <div className="search__data">
            <h3 className="dataHead">
              Search your Email Id and share the information with MICE & More
              Team
            </h3>
        ...
        <div className="belowImage">
          <img src={BelowImageCaho} alt="Caho_Diagnostion_Icon" />
        </div> */}
